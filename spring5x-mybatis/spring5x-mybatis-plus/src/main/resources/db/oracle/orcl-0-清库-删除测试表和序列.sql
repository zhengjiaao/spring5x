...
--===============20191203==================start
    ...
    select count(*) into countTab from user_tables where table_name = upper('student');
    if countTab = 1 then
    ...
    select count(*) into countSeq from user_sequences where sequence_name = 'SEQ_MY_STUDENT';
    if countSeq = 1 then
        execute immediate 'DROP SEQUENCE SEQ_MY_STUDENT';
    end if;
--===============20191203==================end
end;$$
-- delimiter ;